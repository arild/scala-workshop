...
class MyIntroProblemsTest extends FlatSpecHelper with ShouldMatchers {
  ...
  it should "compute square" in {
    checkImplemented {
      MyIntroProblems.computeSquare(4) should equal (16)
    }
  }

  it should "find last element" in {
    checkImplemented {
      MyIntroProblems.findLastElement(List(1, 2, 3)) should equal (3)
    }
  }

  it should "find two last elements" in {
    checkImplemented {
  ...
  it should "compute sum of all max factors" in {
  ...
  it should "compute square of int or string" in {
    checkImplemented {
